...
def score_text_legibility(bgr, doc_poly, save_debug_dir=None, base_name=""):
    ...
    # Metrics computed primarily on text strokes
    sharp = _variance_of_laplacian(text_norm)*(text_mask.mean()/255.0+1e-6)
    edges = float((cv2.Canny(text_norm,60,180)&text_mask>0).mean())
    contr = float(np.std(text_norm[text_mask>0].astype(np.float32)/255.0)) if np.any(text_mask>0) else 0.0
    ...
    return dict(score=round(score,1),label=label,
                sharpness_vlap=round(sharp,1),edge_density=round(edges,4),
                ...
                text_pixels_frac=round(float(text_mask.mean()/255.0),3))
    ...
    if want_json:
        ...
        lines.append(
            ...
            + f" censor_frac={t['censor_area_frac']} blockiness={t['blockiness']} "
              f"mser/MP={t['mser_per_megapixel']} coverage={t['text_coverage']} text_px={t['text_pixels_frac']}"
        )
        return "\n".join(lines)
