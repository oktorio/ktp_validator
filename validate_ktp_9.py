...
# validate_ktp_9.py
# Changelog vs v8:
# - Added extra penalty multiplier based on text legibility label (poor/fair)
# - Boosted color KTP weight (color_document) to give more credit for real-color cards
# - Final JSON now includes separate multipliers (doc vs text) and combined reason
...
# ---- NEW: Final-score multipliers ----
DOC_MULTIPLIERS = {
    "color_document": 1.30,  # boosted to give more weight to KTP warna
    "grayscale_document": 0.20,
    "photocopy_on_colored_background": 0.20,
    "black": 0.00,
    "white": 0.00,
}
# Penalize unclear text at the final stage as well (on top of raw legibility penalties)
...
def main():
    ap = argparse.ArgumentParser(description="KTP photocopy/color detection + value-text legibility + Final Score (0..100) with color boost and unclear-text penalty")
