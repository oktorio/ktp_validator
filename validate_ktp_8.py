...
def analyze_file(path, verbose=False, want_json=True, save_debug=None):
    ...
    # ----- Final Score (down-weights grayscale/photocopy) -----
    # Multipliers can be tuned per your policy
    doc_multipliers = {
        "color_document": 1.50,
    ...
    mult = doc_multipliers.get(doc_label, 1.0)
    final_score = float(np.clip(text["score"] * mult, 0, 100))
    ...
    if want_json:
        ...
        lines.append(
            ...
            f"[x{f['multiplier']} due to {f['reason']}]"
        )
        return "\n".join(lines)

def main():
    ap = argparse.ArgumentParser(description="KTP photocopy/color detection + value-text legibility + Final Score (0..100)")
