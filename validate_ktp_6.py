...
def _value_mask_from_ocr(gray):
    """
    Build a mask covering ONLY the value text to the right of ':' on each line.
    Returns (mask_uint8, boxes_count). If OCR not available or fails, returns (None, 0).
    """
    if not _HAS_TESS:
        return None, 0
    try:
    ...
    # group tokens by (block_num, par_num, line_num)
    ...
    while i < n:
        ...
        for k in idxs:
            ...
            if ":" in t:
                ...
                break
        # if colon found, paint all tokens to its right
        if colon_x is not None:
            for k in idxs:
                x = data["left"][k]; y = data["top"][k]; w = data["width"][k]; h = data["height"][k]
                if x > colon_x + 2:  # to the right of colon
...
def _value_mask_by_projection(text_mask):
    """
    Fallback when OCR is unavailable: find a vertical split and keep right side.
    Prefers the densest third on the right.
    """
    ...
    col /= (col.max()+1e-6)
    W = len(col)
    # choose split between 35%..65% where right density is highest
...
def score_text_legibility(bgr, doc_poly, save_debug_dir=None, base_name=""):
    ...
    sharp = _variance_of_laplacian(text_norm) * (value_frac + 1e-6)  # weight by how much we actually scored
    ...
    # Structural penalties (computed on the normalized text ROI)
    ...
    return dict(
        ...
        text_pixels_frac=round(text_frac,3),
        value_pixels_frac=round(value_frac,3),
        value_boxes=(int(nboxes) if value_mask_ocr is not None else None)
    )
...
def analyze_file(path, verbose=False, want_json=True, save_debug=None):
    ...
    if want_json:
        ...
        lines.append(
            ...
            + f" censor_frac={t['censor_area_frac']} blockiness={t['blockiness']} "
              f"mser/MP={t['mser_per_megapixel']} coverage={t['text_coverage']} "
              f"text_px={t['text_pixels_frac']} value_px={t['value_pixels_frac']} "
              f"value_boxes={t['value_boxes']}"
        )
        return "\n".join(lines)

def main():
    ap = argparse.ArgumentParser(description="KTP photocopy/color detection + value-text legibility score (0..100)")
