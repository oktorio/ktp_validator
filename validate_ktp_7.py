...
def _value_mask_from_ocr(gray):
    """
    Mask covering ONLY the value text to the right of ':' on each line.
    Returns (mask_uint8, boxes_count). If OCR not available or fails -> (None, 0).
    """
    if not _HAS_TESS:
        return None, 0
    try:
...
def _value_mask_by_projection(text_mask):
    """Fallback when OCR unavailable: keep the densest right-side region."""
...
def _expand_value_mask(mask):
    """Expand value mask horizontally so each detected box captures its full line."""
    if mask is None:
        return None
...
def score_text_legibility(bgr, doc_poly, save_debug_dir=None, base_name=""):
    ...
    # Structural penalties (computed on normalized text ROI)
