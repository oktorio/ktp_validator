...
# Legibility score (base metrics) weights
...
CENSOR_MIN_AREA_FRAC   = 0.010   # ≥1% of text ROI area -> censor box
CENSOR_UNIFORM_STD_THR = 8.0     # near-uniform box stddev (0..255)
CENSOR_PENALTY_W       = 45.0    # max points to subtract

PIXEL_GRID_SIZES       = [8, 12, 16, 20, 24]
BLOCKINESS_PENALTY_W   = 20.0

MSER_MIN_COUNT_PER_MP  = 160.0   # too few => penalty (missing text)
MSER_PENALTY_W         = 25.0

# ----- NEW penalties (for your noisy/censored case) -----
OCR_LOW_CONF_THR       = 40.0    # median OCR confidence below this => penalty
OCR_LOW_CONF_PENALTY_W = 20.0

MSER_MAX_COUNT_PER_MP  = 1800.0  # too many blobs => noise penalty
MSER_OVER_PENALTY_W    = 20.0

TEXT_COVERAGE_RANGE    = (0.02, 0.10)  # expected ink coverage (2–10%)
TEXT_COVERAGE_PENALTY_W= 20.0
# ========================================================
...
# ----- Censor / pixelation / text-density helpers -----
...
def _mser_text_density(gray):
    ...
    Uses MSER_create() with runtime-safe setters; falls back to a heuristic.
    """
    h, w = gray.shape[:2]
...
def _text_coverage(gray):
    """Approx. fraction of 'ink' after cleanup (0..1)."""
    thr = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                                cv2.THRESH_BINARY_INV, 35, 10)
...
def score_text_legibility(bgr, doc_poly, save_debug_dir=None, base_name=""):
    ...
    # extra structural signal
    coverage = _text_coverage(text_norm)  # 0..1
    ...
    # NEW: very low ink coverage
    cov_lo, cov_hi = TEXT_COVERAGE_RANGE
    if coverage < cov_lo:
    ...
    if save_debug_dir:
        ...
        cv2.imwrite(os.path.join(save_debug_dir, f"{dbg}_textnorm.png"), text_norm)
